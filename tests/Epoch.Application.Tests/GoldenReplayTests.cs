...
namespace Epoch.Application.Tests
{
    ...
    public static class GoldenReplayTests
    {
        ...
        public static void EmitGoldens()
        {
            ...
            json.Append("  \"note\": \"Complete-run goldens. The two sides use deliberately different legal-choice policies so that scoring, ownership and contested income are actually exercised; neither reads future offers nor consumes RNG, so a run is a pure function of seed and content. Never regenerate these to make a failing test pass: a change here means a rules, content or engine-behaviour change, and needs saying so out loud.\",\n");
            json.Append("  \"regeneratedAt\": \"M3\",\n");
            json.Append("  \"regenerationReason\": \"Regenerated twice, both owner-sanctioned. M2: contentHash replaced the placeholder sha256:pending-m2 with the canonical hash (contentPoolHash is authoritative state), and five authored effects that M1 loaded but never executed began applying - PERK_ECO_RIVER_GROWTH, PERK_UTIL_COAST_GROWTH, PERK_UTIL_HIGHLAND_INSIGHT, PERK_CONV_RIVER_INSIGHT and BUILD_GARRISON_POST. M3: the stack is now ranked on effect-modified pre-stacking Power rather than raw basePower, completing owner ruling (L) - a UNIT_CLASS Power effect may change stack rank, not only the dominant class. No rule, Age table, authored value or balance number was changed by either.\",\n");
            json.Append("  \"runs\": [\n");
